...
public class BoardService {
	...
	public void insertReply(BoardVo boardVo) {
		boardDao.insertReply(boardVo.getTitle(), boardVo.getContent(), boardVo.getGroupNo(), boardVo.getOrderNo(),
				boardVo.getDepth(), boardVo.getUserNo());
	}
	...
	public int getSearchCount(String search, String kwd) {
		return boardDao.getSearchCount(search, kwd);
	}

	public Map<String, Object> getSearch(String search, String kwd, int page) {
		// pager 알고리즘
		PageVo vo = new PageVo();
		List<BoardVo> list = boardDao.getSearch(search, kwd, 1);
		
		vo.setTotalCount(boardDao.getSearchCount(search, kwd));
		int totalPage = vo.getTotalPage(vo.getTotalCount());
		vo.setStartPage(1);
		vo.setEndPage(10);
		vo.setPage(page);
		...
		if(vo.getTotalCount() > 10) {
			...
			vo.setPage(page);
			list = boardDao.getSearch(search, kwd, vo.getPage());
		}
